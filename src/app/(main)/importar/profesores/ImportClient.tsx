...
import { insertProfesores } from './actions'
...
export default function ImportClient({ initialCount, lastUpdate }: { initialCount: number; lastUpdate: string | null }) {
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (event) => {
            ...
            let startIdx = 0
            if (lines[0].toLowerCase().includes('profesor') && lines[0].toLowerCase().includes('correo')) {
                startIdx = 1
            }
            ...
            for (let i = startIdx; i < lines.length; i++) {
                ...
                // Support both comma and semicolon, respecting double quotes ("")
                ...
                if (parts.length >= 3) {
                    const profesor = parts[0]
                    const correo = parts[1]
                    const puesto = parts[2]
                    const fecha_cese = parts[3] ? parts[3] : undefined
                    ...
                    parsedRows.push({
                        ...
                        puesto: '-',
                        valido: false,
                        error: 'Formato inválido (se esperaban al menos 3 columnas)'
                    })
                }
            }
    ...
    const handleImport = async () => {
        ...
        const result = await insertProfesores(validRows)
        ...
        if (result.error) {
            ...
            toast.success(`${result.count} profesores importados correctamente.`)
            setRows([])
            setFile(null)
        }
    }
                    ...
                    <div className="mb-6 p-4 rounded-lg bg-primary-brand/5 border border-primary-brand/20 flex gap-4 items-start">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                                Sube un archivo <strong className="font-semibold">CSV</strong> con las columnas separadas por <strong className="font-semibold">punto y coma (;) o coma (,)</strong>. <br/>Ejemplo: <code className="bg-primary-brand/10 px-1 rounded dark:text-primary-brand">"Profesor";"Correo";"Puesto";"fecha_cese"</code>.
                                <br /><br />
                                <strong className="font-semibold">Se añadirán los profesores nuevos sin borrar los existentes. El sistema evitará duplicar profesores que ya estén registrados usando su correo.</strong>
                            </p>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left relative">
                            <thead>
                                <tr className="bg-slate-50 dark:bg-slate-800/90 backdrop-blur text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">
                                    <th className="px-6 py-4 font-semibold text-left">Profesor</th>
                                    <th className="px-6 py-4 font-semibold text-left">Correo</th>
                                    <th className="px-6 py-4 font-semibold text-left">Puesto</th>
                                    <th className="px-6 py-4 font-semibold text-left">Fecha Cese</th>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                                {rows.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                        <td className="px-6 py-4 font-medium text-slate-900 dark:text-white">{row.profesor}</td>
                                        <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{row.correo}</td>
                                        <td className="px-6 py-4">
                                            <span className="px-2 py-1 rounded bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-medium">
                                                {row.puesto}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{row.fecha_cese || '-'}</td>
