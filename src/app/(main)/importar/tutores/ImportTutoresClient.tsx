...
import { insertTutores } from './actions'
...
export default function ImportTutoresClient({ initialCount, lastUpdate }: { initialCount: number; lastUpdate: string | null }) {
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (event) => {
            ...
            // Detectar cabecera para ignorarla si existe
            if (lines[0].toLowerCase().includes('curso') || lines[0].toLowerCase().includes('correo')) {
                startIdx = 1
            }
            ...
            for (let i = startIdx; i < lines.length; i++) {
                ...
                // Detectar el separador a usar (priorizar coma si existe, sino punto y coma)
                let separator = ';'
                if (line.includes(',')) {
                    separator = ','
                }
                ...
                if (parts.length >= 2) {
                    ...
                    if (!isEmailValid) {
                        ...
                        parsedRows.push({
                            ...
                            correo,
                            valido: false,
                            error: 'Falta especificar el curso'
                        })
                    } else {
                        parsedRows.push({
                            curso,
                    ...
                    parsedRows.push({
                        ...
                        correo: '-',
                        valido: false,
                        error: 'Formato inválido (se esperaba ; o , indicando 2 columnas)'
                    })
                }
            }
    ...
    const handleImport = async () => {
        ...
        // Payload a procesar en backend (simulado por ahora)
        const payload = validRows.map(r => ({
            curso: r.curso,
            correo: r.correo,
        }))

        // Llama a actions.ts temporal (sin BD hasta confirmar)
        const result = await insertTutores(payload)
        ...
        if (result.error) {
            ...
            toast.success(`${result.count} tutores importados/actualizados correctamente en el sistema.`)
            setRows([])
            setFile(null)
        }
    }
                    ...
                    <div className="mb-6 p-4 rounded-lg bg-primary-brand/5 border border-primary-brand/20 flex gap-4 items-start">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                                Sube un archivo <strong className="font-semibold">CSV</strong> con las columnas separadas por <strong className="font-semibold">punto y coma (;) o comas (,)</strong> y con los textos delimitados por <strong className="font-semibold">comillas (")</strong>. Ejemplo: <code className="bg-primary-brand/10 px-1 rounded dark:text-primary-brand">"Curso","Correo"</code>.
                                <br /><br />
                                <strong className="font-semibold">El sistema cruzará el nombre de los cursos existentes. Si introduces un curso que aún no existe, lo creará automáticamente con su email de tutor.</strong>
                            </p>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left relative">
                            <thead>
                                <tr className="bg-slate-50 dark:bg-slate-800/90 backdrop-blur text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">
                                    <th className="px-6 py-4 font-semibold text-left">Curso / Unidad</th>
                                    <th className="px-6 py-4 font-semibold text-left">Correo del Tutor</th>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                                {rows.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                        <td className="px-6 py-4 font-medium text-slate-900 dark:text-white">{row.curso}</td>
